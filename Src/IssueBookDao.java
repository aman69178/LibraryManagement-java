...
public static int updatebook(String bookcallno){
	...
	try{
		...
		PreparedStatement ps=con.prepareStatement("select quantity from book where callno=?");
		ps.setString(1,bookcallno);
		ResultSet rs=ps.executeQuery();
		if(rs.next()){
		...
		if(quantity>0){
		PreparedStatement ps2=con.prepareStatement("update book set quantity=? where callno=?");
